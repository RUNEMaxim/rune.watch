...
// ----------------------------------------------------------------------------
// FIX 10: /recent-swaps -- die letzten Swaps für die Live-Partikel-/Live-Chart-Anzeige im
// Frontend. Wird dort alle 7s gepollt -- kurzlebiger Cache (4s), damit bei mehreren
// gleichzeitig aktiven Nutzern nicht jeder einen eigenen Midgard-Request alle 7s auslöst,
// sondern sich mehrere Polls dieselbe, ganz frische Antwort teilen.
// ----------------------------------------------------------------------------
function fetchRecentSwapActions() {
  return fetchFromBases(MIDGARD_BASES, '/actions?type=swap&limit=50');
}
...
async function handleRecentSwaps(request, env) {
  try {
    ...
    return json({ error: 'FETCH_FAILED', message: e?.message || String(e) }, env, 502);
  }
}
